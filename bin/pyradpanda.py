...
def parse_args(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Process radiomics features from nrrd files", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
        "-i",
        "--inputimage",
    ...
    parser.add_argument("-r", "--resume", help="Resume pipeline", default="again", action="store_true")
...
def main(argv=None):
    ...
    if not args.inputimage.is_file():
        logger.error(f"The given input file {args.inputimage} was not found!")
        sys.exit(2)
    else:
    ...
    if (imageFilepath is not None) and (maskFilepath is not None):
        dict = {"Image": os.path.basename(imageFilepath), "Mask": os.path.basename(maskFilepath)}
